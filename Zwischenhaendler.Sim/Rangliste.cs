...
public class Rangliste
{
    ...
    public void ZeigeRangliste(Bankrott Bankrott)
    {
        ...
        foreach(Zwischenhändler Händler in Globals.Händler)
        {
            ...
            Console.WriteLine(string.Format(Ausgabe, Platzierung, Händler.Name, Händler.Firma, Math.Round(Händler.Kontostand,2)));
            Platzierung++;
        }
        ...
        foreach(Zwischenhändler Händler in Bankrott.AusgeschiedeneHändler)
        {
            ...
            Console.WriteLine(string.Format(Ausgabe, Platzierung, Händler.Name, Händler.Firma, Math.Round(Händler.Kontostand,2), Händler.TagAusscheidung));
            Platzierung++;
        }
    }
}